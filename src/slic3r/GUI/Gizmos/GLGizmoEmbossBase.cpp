...
// Loaded icons enum
// Have to match order of files in function GLGizmoEmbossBase::init_icons()
...
static GLGizmosManager::EType to_gizmo_manager_etype(GLGizmoEmbossBase::EmbossType emboss_type)
{
    switch (emboss_type) {
    case GLGizmoEmbossBase::EmbossType::EMBOSS:
        return GLGizmosManager::EType::Emboss;
    case GLGizmoEmbossBase::EmbossType::REVISION:
        return GLGizmosManager::EType::RevisionEmboss;
    default:
        assert(false);
        return GLGizmosManager::EType::Emboss;
    }
...
GLGizmoEmbossBase::GLGizmoEmbossBase(GLCanvas3D &parent, EmbossType emboss_type, int shortcut_key)
    ...
    , m_rotate_gizmo(parent, GLGizmoRotate::Axis::Z) // grab id = 2 (Z axis)
{
    ...
    m_shortcut_key = shortcut_key;
    m_emboss_type = emboss_type;
}

bool GLGizmoEmbossBase::create_volume(ModelVolumeType volume_type, const Vec2d& mouse_pos)
{
...
bool GLGizmoEmbossBase::create_volume(ModelVolumeType volume_type)
{
    if (!init_create(volume_type))
        return false;
...
CreateVolumeParams GLGizmoEmbossBase::create_input(ModelVolumeType volume_type)
{
    ...
    auto gizmo = static_cast<unsigned char>(to_gizmo_manager_etype(m_emboss_type));
...
void GLGizmoEmbossBase::on_shortcut_key() {
    ...
    if (m_volume == nullptr) {
        ...
        if (mng.get_current_type() != to_gizmo_manager_etype(m_emboss_type))
            mng.open_gizmo(to_gizmo_manager_etype(m_emboss_type));
    }
}

bool GLGizmoEmbossBase::re_emboss(const ModelVolume &text_volume, std::shared_ptr<std::atomic<bool>> job_cancel)
{
...
bool GLGizmoEmbossBase::do_mirror(size_t axis)
{ 
    // is valid input
    assert(axis < 3);
    if (axis >= 3)
    ...
    assert(m_parent.get_gizmos_manager().get_current_type() == to_gizmo_manager_etype(m_emboss_type));
    if (m_parent.get_gizmos_manager().get_current_type() != to_gizmo_manager_etype(m_emboss_type))
        return false;
...
bool GLGizmoEmbossBase::init_create(ModelVolumeType volume_type)
{
    ...
    return setup_dialog_defaults();
}

bool GLGizmoEmbossBase::on_mouse_for_rotation(const wxMouseEvent &mouse_event)
{
...
bool GLGizmoEmbossBase::on_mouse_for_translate(const wxMouseEvent &mouse_event)
{
...
void GLGizmoEmbossBase::on_mouse_change_selection(const wxMouseEvent &mouse_event)
{
...
bool GLGizmoEmbossBase::on_mouse(const wxMouseEvent &mouse_event)
{
    // not selected volume
    if (m_volume == nullptr ||
...
void GLGizmoEmbossBase::volume_transformation_changing()
{
...
void GLGizmoEmbossBase::volume_transformation_changed()
{
    if (m_volume == nullptr || 
...
bool        GLGizmoEmbossBase::wants_enter_leave_snapshots() const { return true; }
std::string GLGizmoEmbossBase::get_gizmo_entering_text() const { return _u8L("Enter emboss gizmo"); }
std::string GLGizmoEmbossBase::get_gizmo_leaving_text() const { return _u8L("Leave emboss gizmo"); }
std::string GLGizmoEmbossBase::get_action_snapshot_name() const { return _u8L("Embossing actions"); }

bool GLGizmoEmbossBase::on_init()
{
    m_rotate_gizmo.init();
...
void GLGizmoEmbossBase::on_render() {
    // no volume selected
    const Selection &selection = m_parent.get_selection();
    if (m_volume == nullptr ||
...
void GLGizmoEmbossBase::on_register_raycasters_for_picking(){
    m_rotate_gizmo.register_raycasters_for_picking();
}
void GLGizmoEmbossBase::on_unregister_raycasters_for_picking(){
...
void GLGizmoEmbossBase::on_render_input_window(float x, float y, float bottom_limit)
{
...
void GLGizmoEmbossBase::on_set_state()
{
...
void GLGizmoEmbossBase::data_changed(bool is_serializing) {
    set_volume_by_selection();
    if (!is_serializing && m_volume == nullptr)
        close();
}

void GLGizmoEmbossBase::on_start_dragging() { m_rotate_gizmo.start_dragging(); }
void GLGizmoEmbossBase::on_stop_dragging()
{
...
void GLGizmoEmbossBase::on_dragging(const UpdateData &data) { m_rotate_gizmo.dragging(data); }

EmbossStyles GLGizmoEmbossBase::create_default_styles()
{
    wxFontEnumerator::InvalidateCache();
...
void GLGizmoEmbossBase::reinit_text_lines(unsigned count_lines) {    
    init_text_lines(m_text_lines, m_parent.get_selection(), m_style_manager, count_lines);
}

void GLGizmoEmbossBase::set_volume_by_selection()
{
    const Selection &selection = m_parent.get_selection();
    const GLVolume  *gl_volume = get_selected_gl_volume(selection);
    if (gl_volume == nullptr)
...
void GLGizmoEmbossBase::reset_volume()
{
    if (m_volume == nullptr)
...
void GLGizmoEmbossBase::calculate_scale() {
...
bool GLGizmoEmbossBase::process(bool make_snapshot, std::optional<Transform3d> volume_transformation) {
...
void GLGizmoEmbossBase::draw_text_input()
{
...
void GLGizmoEmbossBase::init_font_name_texture() {
...
bool GLGizmoEmbossBase::select_facename(const wxString &facename)
{
...
void GLGizmoEmbossBase::draw_font_list_line()
{    
...
void GLGizmoEmbossBase::draw_font_list()
{
    // Set partial
...
void GLGizmoEmbossBase::draw_model_type()
{
...
void GLGizmoEmbossBase::draw_style_rename_popup() {
...
void GLGizmoEmbossBase::draw_style_rename_button()
{
...
void GLGizmoEmbossBase::draw_style_save_button(bool is_modified)
{
...
void GLGizmoEmbossBase::draw_style_save_as_popup() {
...
void GLGizmoEmbossBase::draw_style_add_button()
{
...
void GLGizmoEmbossBase::draw_delete_style_button() {
...
void GLGizmoEmbossBase::draw_style_list() {
...
bool GLGizmoEmbossBase::draw_italic_button()
{
    const wxFont &wx_font = m_style_manager.get_wx_font(); 
...
bool GLGizmoEmbossBase::draw_bold_button() {
    const wxFont &wx_font = m_style_manager.get_wx_font();
    const auto& ff = m_style_manager.get_font_file_with_cache();
    if (!wx_font.IsOk() || !ff.has_value()) {
...
bool GLGizmoEmbossBase::revertible(const std::string &name,
                               T                 &value,
...
template<typename T> 
bool GLGizmoEmbossBase::rev_input(const std::string &name, T &value, const T *default_value,
...
template<typename T>
bool GLGizmoEmbossBase::rev_input_mm(const std::string          &name,
                                 T                          &value,
...
bool GLGizmoEmbossBase::rev_checkbox(const std::string &name,
                                 bool              &value,
...
bool GLGizmoEmbossBase::set_height() {
...
void GLGizmoEmbossBase::draw_height(bool use_inch)
{
    float &value = m_style_manager.get_font_prop().size_in_mm;
    const EmbossStyle* stored_style = m_style_manager.get_stored_style();
...
void GLGizmoEmbossBase::draw_depth(bool use_inch)
{
...
bool GLGizmoEmbossBase::rev_slider(const std::string &name,
...
void GLGizmoEmbossBase::draw_advanced()
{
    const auto &ff = m_style_manager.get_font_file_with_cache();
    if (!ff.has_value()) { 
    ...
    //ImGui::SameLine(); if (ImGui::Button("Re-emboss")) GLGizmoEmbossBase::re_emboss(*m_volume);    
...
void GLGizmoEmbossBase::draw_use_surface(){
...
void GLGizmoEmbossBase::draw_per_glyph() {
...
void GLGizmoEmbossBase::draw_align()
{     
...
void GLGizmoEmbossBase::draw_char_gap()
{
...
void GLGizmoEmbossBase::draw_line_gap() {
    const FontFile &ff = *m_style_manager.get_font_file_with_cache().font_file;
    const FontProp &fp = m_style_manager.get_font_prop();
    const FontFile::Info &font_info = get_font_info(ff, fp);
...
void GLGizmoEmbossBase::draw_boldness(){
...
void GLGizmoEmbossBase::draw_skew()
{
...
void GLGizmoEmbossBase::draw_surface_distance()
{
    const StyleManager::Style &current_style = m_style_manager.get_style();
...
void GLGizmoEmbossBase::draw_rotation() {
...
void GLGizmoEmbossBase::set_minimal_window_size(bool is_advance_edit_style)
{
...
ImVec2 GLGizmoEmbossBase::get_minimal_window_size() const
{
    ImVec2 res;
...
#ifdef ALLOW_ADD_FONT_BY_OS_SELECTOR
bool GLGizmoEmbossBase::choose_font_by_wxdialog()
{
    wxFontData data;
...
#ifdef ALLOW_ADD_FONT_BY_FILE
bool GLGizmoEmbossBase::choose_true_type_file()
{
...
void GLGizmoEmbossBase::create_notification_not_valid_font(
...
void GLGizmoEmbossBase::create_notification_not_valid_font(const std::string &text) {
...
void GLGizmoEmbossBase::remove_notification_not_valid_font()
{
...
void GLGizmoEmbossBase::init_icons()
{
...
const std::string GLGizmoEmbossBase::M_ICON_FILENAME = "cut.svg";